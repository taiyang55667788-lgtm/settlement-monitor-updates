...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const queryHtml = `<button>本星期</button><button onclick="showRoot()">查 询</button><main id="report"></main><script>
      function showRoot() { document.querySelector('#report').innerHTML = ${JSON.stringify(rootTable)}; }
      function showChildren(parent) { document.querySelector('#report').innerHTML = parent === 'parent-01' ? ${JSON.stringify(childTableA)} : ${JSON.stringify(childTableB)}; }
    </script>`;
    fs.writeFileSync(path.join(directory, 'query.html'), queryHtml);
    fs.writeFileSync(path.join(directory, 'index.html'), '<a href="./query.html">报表查询</a><iframe id="frame" name="frame"></iframe>');
